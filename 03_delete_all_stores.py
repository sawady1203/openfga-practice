...
if __name__ == "__main__":
    ...
    for store in stores:
        store_url = f"{FGA_API_URL}/stores/{store["id"]}"
        deleted = send_request(method="DELETE", url=store_url, payload=None, headers=headers)
        if deleted:
            print("ステータスコード:", response.status_code)
            try:
                pprint(deleted.json(),indent=2)
            except json.JSONDecodeError:
                print("レスポンス (テキスト形式):", deleted.text)
